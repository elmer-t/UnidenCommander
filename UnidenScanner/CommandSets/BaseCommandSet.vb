...
#Region " Command response handlers "
        ...
        Protected Sub Line1CallbackHandler(ByVal response As String)
            ...
            'Dim rx As Match = Regex.Match(response, "\[(.+)\s*([+-])\s*([P ])\s*C\s*(\d{3})(\s*|Lo)\]")
            
            ' Groups:
            ' 1: scan (mode)
            ' 2: + / - (direction)
            ' 3: P or empty (priority)
            ' 4: 123 (channel)
            ' 5: Lo or empty (lock out)

            Dim tmp As String
            tmp = Regex.Match(response, "\[(.+)\s*([+-])(.+)\]").Groups(2).Value
            If Not String.IsNullOrEmpty(tmp) Then
                If tmp = "+" Then ScannerReference.SetScanDirection(ScanDirections.Up) Else ScannerReference.SetScanDirection(ScanDirections.Down)
            End If

            tmp = Regex.Match(response, "\[(.+)\s*( P )(.+)\]").Groups(2).Value
            If Not String.IsNullOrEmpty(tmp) Then
                ScannerReference.SetPriorityChannel(tmp = " P ")
            Else
                ScannerReference.SetPriorityChannel(False)
            End If

            tmp = Regex.Match(response, "C\s*(\d{3})(\s*|Lo)\]").Groups(1).Value
            If Not String.IsNullOrEmpty(tmp) Then
                Dim ch As Integer
            ...
            tmp = Regex.Match(response, "C\s*(\d{3})(\s*|Lo)\]").Groups(2).Value
            If Not String.IsNullOrEmpty(tmp) Then
                ScannerReference.SetLockOut(tmp = "Lo")
            End If
